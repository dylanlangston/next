...
<div id="gamepad" in:fade={{ delay: 250, duration: 300 }} out:fade>
	<div
		...
		class="absolute bottom-4 left-4 z-10 m-auto p-1 grid grid-cols-2 grid-rows-1 w-fit h-fit items-center justify-items-center bg-main/[.5] rounded-full select-none touch-none"
	>
		<button
			id="right"
			title="Right"
			class="row-start-1 col-start-2 bg-main/[.5] rounded-r-full text-black"
			value={Button.Right}><i class="arrow right" /></button
		>
		<button
			id="left"
			title="Left"
			class="row-start-1 col-start-1 bg-main/[.5] rounded-l-full text-black"
	...
	<div
		...
		class="absolute bottom-4 right-4 z-10 m-auto p-1 grid grid-cols-1 grid-rows-2 w-fit h-fit items-center justify-items-center bg-main/[.5] rounded-full select-none touch-none"
	>
		<button
			id="up"
			title="Up"
			class="row-start-1 col-start-1 bg-main/[.5] rounded-t-full text-black"
			value={Button.Up}><i class="arrow up" /></button
		>
		<button
			id="down"
			title="Down"
			class="row-start-2 col-start-1 bg-main/[.5] rounded-b-full text-black"
			value={Button.Down}><i class="arrow down" /></button
		>
	</div>

	<div
		class="absolute bottom-4 right-28 z-10 bg-main/[.5] rounded-full p-1 w-fit h-fit m-auto select-none"
	>
		<button
			id="a"
			title="A"
			class="bg-main/[.5] rounded-full w-24 h-24 p-0 font-bold text-black"
			value={Button.A}
			...
			on:lostpointercapture={(e) => setTimeout(() => touchUp(e))}>A</button
		>
	</div>

	<div
		class="absolute bottom-28 left-4 z-10 bg-main/[.5] rounded-full p-1 w-fit h-fit select-none"
	>
		<button
			id="start"
			title="Start"
			class="bg-main/[.5] rounded-full w-14 h-8 p-0 font-bold text-black"
			value={Button.Start}
			on:pointerdown={(e) => touchDown(e)}
			on:pointerup={(e) => setTimeout(() => touchUp(e))}
			on:pointerleave={(e) => setTimeout(() => touchUp(e))}
			on:pointercancel={(e) => setTimeout(() => touchUp(e))}
			on:lostpointercapture={(e) => setTimeout(() => touchUp(e))}>Start</button
		>
	</div>
</div>
